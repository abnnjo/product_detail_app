...
class ProductAPI:
	...
	async def get_product_detail(self, product_id: str) -> RowProxy:
		...
		if not result:
			raise DataDoesNotExist(f"Data does not exist for product with id '{product_id}'")
		return dict(result)
	...
	async def create_product(self, data: Dict):
		...
		cat_id, bra_id = await asyncio.gather(self.create_category(category_data), self.create_brand(brand))
	...
	async def create_category(self, data) -> bool:
		"""
		"""
		query = select([Category.id]).where(func.lower(Category.name) == data.get("name", ""))
